...
HWSerial::HWSerial()
{
    ...
    GpioSetDir(GPORT3_BIT4, OUTPUT);
    GpioSetDir(GPORT3_BIT5, OUTPUT);
    GpioWrite(GPORT3_BIT4, LOW);
    GpioWrite(GPORT3_BIT5, LOW);
    ...
    hSerial1 = CreateFile(portName1,          // Specify port device: default "COM1"
    ...
    config1.BaudRate = 19200;                 // Specify buad rate of communicaiton.
    config1.StopBits = ONESTOPBIT;           // Specify stopbit of communication (1 stop bit).
    config1.Parity = NOPARITY;               // Specify parity of communication (parity none).
    config1.ByteSize = 8;                    // Specify  byte of size of communication (8 bit).

    if (SetCommState(hSerial1, &config1) == 0)
    {
...
void HWSerial::begin(int baud_rate)
{
    ...
    // Assign user parameter.
    config1.BaudRate = baud_rate;            // Specify buad rate of communicaiton.
    config1.StopBits = ONESTOPBIT;           // Specify stopbit of communication (1 stop bit).
    config1.Parity = NOPARITY;               // Specify parity of communication (parity none).
    config1.ByteSize = 8;                    // Specify  byte of size of communication (8 bit).
...
size_t HWSerial::write(uint8_t byte)
{
    ...
    //return Serial1.write(byte);
    if (WriteFile(hSerial1,			// handle of file to read
        &byte,						// handle of file to read
...
int HWSerial::read()
{
    ...
    if (ReadFile(hSerial1,         // handle of file to read
...
//size_t HWSerial::print(const char *ifsh)
//{
//  const prog_char *p = (const prog_char *)ifsh;
//  size_t n = 0;
//  while (1) {
...
size_t HWSerial::print(const std::string &s)
{
    size_t n = 0;
    for (uint16_t i = 0; i < s.length(); i++) {
        n += write(s[i]);
...
size_t HWSerial::println(const std::string &s)
{
    size_t n = print(s);
    n += println();
    return n;
