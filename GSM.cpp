...
Modified to be compatible with Intel Galileo running Windows 
*/
...
#else
          printf("ERROR: SIM900 doesn't answer. Check power and serial pins in GSM.cpp");
#endif
          digitalWrite(GSM_ON, HIGH);
          delay(1200);
          digitalWrite(GSM_ON, LOW);
                            ...
                            byte no_of_attempts)
{
     ...
     for (i = 0; i < no_of_attempts; i++) {
          ...
          if (status == RX_FINISHED) {
               ...
               // due to the first implementation of hardware serial 
               ret_val = AT_RESP_OK;		//MOD Dacol
               break;						//MOD Dacol
#endif
          } else {
               // nothing was received
